...
{
    public class CompanyAccountFilters<T> : AccountFilters<T> where T : Account
    {
        public IList<T> FilterAccounts(IEnumerable<T> comptes, ListAccountsRequest criterias, IConfiguration configuration)
        {
            ...
            {
                if (matchName(((CompanyAccount)(Account)account), criterias, decryptCriteria) && matchTown(((CompanyAccount)(Account)account), criterias, decryptCriteria)
                    && matchPostalCode(((CompanyAccount)(Account)account), criterias, decryptCriteria) && matchCounty(((CompanyAccount)(Account)account), criterias, decryptCriteria))
                {
        ...
        private bool matchName(CompanyAccount account, ListAccountsRequest criterias, StringEncryptCriteria decryptCriteria)
        {
        ...
        /// <param name="decryptCriteria">Criteres de déchiffrage</param>
        /// <returns>bool : true si critere vide ou correspondant, false sinon</returns>
        private bool matchTown(CompanyAccount account, ListAccountsRequest criterias, StringEncryptCriteria decryptCriteria)
        {
        ...
        /// <param name="decryptCriteria">Criteres de déchiffrage</param>
        /// <returns>bool : true si critere vide ou correspondant, false sinon</returns>
        private bool matchPostalCode(CompanyAccount account, ListAccountsRequest criterias, StringEncryptCriteria decryptCriteria)
        {
            decryptCriteria.Text = account.Adress.PostalCode;
            // si le nom de la ville ne contient pas le nom en critere
            if (!string.IsNullOrEmpty(criterias.PostalCode) && !StringEncryptHelper.Decrypt(decryptCriteria).Equals(criterias.PostalCode.ToUpper()))
            {
                return false;
            }
            return true;
        ...
        /// <param name="decryptCriteria">Criteres de déchiffrage</param>
        /// <returns>bool : true si critere vide ou correspondant, false sinon</returns>
        private bool matchCounty(CompanyAccount account, ListAccountsRequest criterias, StringEncryptCriteria decryptCriteria)
        {
            decryptCriteria.Text = account.Adress.PostalCode;
            // si le nom de la ville ne contient pas le nom en critere
